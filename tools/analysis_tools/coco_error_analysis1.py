...
def find_category_errors(cocoGt, cocoDt, iou_type, iou_thr=0.5):
    catId_to_name = {cat['id']: cat['name'] for cat in cocoGt.loadCats(cocoGt.getCatIds())}
    imgId_to_name = {img['id']: img['file_name'] for img in cocoGt.loadImgs(cocoGt.getImgIds())}
    category_errors = []

    for imgId in cocoGt.getImgIds():
        annIds = cocoGt.getAnnIds(imgIds=[imgId])
        all_gt_anns = cocoGt.loadAnns(annIds)
        gt_ignore = [g.get('ignore', 0) or g.get('iscrowd', 0) for g in all_gt_anns]
        non_ignore_indices = [i for i, ignore in enumerate(gt_ignore) if not ignore]
        gt_anns = [all_gt_anns[i] for i in non_ignore_indices]

        dt_anns = cocoDt.loadAnns(cocoDt.getAnnIds(imgIds=[imgId]))

        if len(gt_anns) == 0 or len(dt_anns) == 0:
            continue

        cocoEval = COCOeval(cocoGt, cocoDt, iou_type)
        iou = cocoEval.computeIoU(imgId)  # shape (len(dt_anns), len(all_gt_anns))

        for d, dt in enumerate(dt_anns):
            if len(non_ignore_indices) > 0:
                iou_non_ignore = iou[d, non_ignore_indices]
                if len(iou_non_ignore) > 0:
                    max_iou = np.max(iou_non_ignore)
                    max_iou_idx = non_ignore_indices[np.argmax(iou_non_ignore)]
                    if max_iou > iou_thr:
                        gt = all_gt_anns[max_iou_idx]
                        if dt['category_id'] != gt['category_id']:
                            category_errors.append({
                                'image_name': imgId_to_name[imgId],
                                'predicted_category': catId_to_name[dt['category_id']],
                                'true_category': catId_to_name[gt['category_id']],
                                'iou': max_iou,
                                'detection_id': dt['id'],
...
def main():
    ...
    if args.score_thr:
        cocoDt.dataset['annotations'] = list(
            filter(lambda ann: ann['score'] >= args.score_thr,
                   cocoDt.dataset['annotations']))
        cocoDt.createIndex()
    ...
    for res_type in args.types:
        print(f"Analyzing category errors for {res_type}")
        errors = find_category_errors(cocoGt, cocoDt, res_type, iou_thr=0.5)
        with open(os.path.join(args.out_dir, f"{res_type}_category_errors.txt"), 'w') as f:
            errors_by_image = defaultdict(list)
            for error in errors:
                errors_by_image[error['image_name']].append((error['predicted_category'], error['true_category']))
            for image_name in sorted(errors_by_image.keys()):
                f.write(f"Image: {image_name}\n")
                for pred_cat, true_cat in errors_by_image[image_name]:
                    f.write(f"- Predicted: {pred_cat}, True: {true_cat}\n")
                f.write("\n")
