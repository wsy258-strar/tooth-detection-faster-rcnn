...
def plot_confusion_matrix(confusion_matrix,
    ...
    fig, ax = plt.subplots(
        figsize=(0.4 * num_classes, 0.4 * num_classes * 0.8), dpi=300)
    ...
    label_font = {'size': 20}
    plt.ylabel('Ground Truth Label', fontdict=label_font, labelpad=-30)
    ...
    fig.tight_layout()
    # plt.subplots_adjust(right=0.8)  # 添加这一行来调整右侧边距
    if save_dir is not None:
        plt.savefig(
            os.path.join(save_dir, 'confusion_matrix.jpg'), format='jpg')
    if show:
        plt.show()
