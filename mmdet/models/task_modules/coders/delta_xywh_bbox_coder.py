...
class DeltaXYWHBBoxCoder(BaseBBoxCoder):
    ...
    Args:
        target_means 和 target_stds 是用于目标检测任务中边界框（bounding box）回归的参数,可以看RCNN中的tx,ty,tw,th公式。
...
def bbox2delta(
    ...
    gw = gt[..., 2] - gt[..., 0]
    gh = gt[..., 3] - gt[..., 1]
    # dx,dy 预测值表示 gt bbox 中心相对于 anchor 中心点的偏移，并且通过除以 anchor 的 wh 进行归一化
    dx = (gx - px) / pw
    dy = (gy - py) / ph
    # dw,dh 预测值表示 gt bbox 的 wh 除以 anchor 的 wh，然后取 log 非线性变换即可。
    dw = torch.log(gw / pw)
    dh = torch.log(gh / ph)
