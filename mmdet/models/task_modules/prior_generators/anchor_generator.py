...
class AnchorGenerator:
    ...
    Args:
        ...
        ratios比例 (list[float]): The list of ratios between the height and width
            of anchors in a single level.
        scales尺度 (list[int], Optional): Anchor scales for anchors
    ...
    def gen_single_level_base_anchors(self,
        ...
        Args:
            ...
            scales (torch.Tensor): Scales of the anchor.    尺度(3种)
        ...
        if self.scale_major:
            ws = (w * w_ratios[:, None] * scales[None, :]).view(-1) # 无论原张量的形状如何，view(-1) 会将其所有元素排列成一个一维数组。
        ...
        # torch.stack 将这些坐标堆叠在一起，生成一个张量，形状为 (n, 4)，其中 n 是生成的 anchor 数量。
        base_anchors = torch.stack(base_anchors, dim=-1) # dim=-1表示从
    ...
    # 利用输入特征图尺寸(shift偏移量）加上 base anchors，得到每个特征图位置的对于原图的 anchors
    def single_level_grid_priors(self,
        ...
        Args:
            featmap_size (tuple[int, int]): Size of the feature maps. 特征图的大小
            level_idx (int): The index of corresponding feature map level. 特征图的索引
        ...
        feat_h, feat_w = featmap_size #featmap_size：特征图的大小
        stride_w, stride_h = self.strides[level_idx]
        # 遍历特征图上所有位置，并且乘上 stride，特征图上的坐标映射到原始图像的尺度上
        ...
        shifts = torch.stack([shift_xx, shift_yy, shift_xx, shift_yy], dim=-1) # [左上角偏移量，右上角偏移量]
