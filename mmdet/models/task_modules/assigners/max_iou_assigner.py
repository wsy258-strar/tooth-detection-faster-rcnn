...
class MaxIoUAssigner(BaseAssigner):
    ...
    def assign_wrt_overlaps(self, overlaps: Tensor,
        ...
        # assigned_gt_inds为正负样本标签，0代表负样本，在这全部赋值为 -1，表示当前全部设置为忽略样本
        assigned_gt_inds = overlaps.new_full((num_bboxes, ),
                                             -1,
                                             dtype=torch.long)
        
        # 将每个 anchor 和所有 gt bbox 计算 iou，找出最大 iou，如果该 iou 小于 neg_iou_thr 或者在背景样本阈值范围内，
        # 则该 anchor 对应索引位置的 assigned_gt_inds 设置为 0，表示是负样本(背景样本)
        ...
        max_overlaps, argmax_overlaps = overlaps.max(dim=0) # overlaps (Tensor): Overlaps between k gt_bboxes and n bboxes, shape(k, n).
        ...
        if isinstance(self.neg_iou_thr, float):
            ...
            assert len(self.neg_iou_thr) == 2 # 布尔表达式，为False会触发AssertionError 并停止运行
            # 可以设置一个范围
        ...
        # 计算高质量正样本
        # 将每个 anchor 和所有 gt bbox 计算 iou，找出最大 iou，如果其最大 iou 大于等于 pos_iou_thr，
        # 则设置该 anchor 对应所有的 assigned_gt_inds 设置为当前匹配 gt bbox 的编号 +1(后面会减掉 1)，
        # 表示该 anchor 负责预测该 gt bbox，且是高质量 anchor。之所以要加 1，是为了区分背景样本(背景样本的 assigned_gt_inds 值为 0)
        pos_inds = max_overlaps >= self.pos_iou_thr # pos_inds 是一个布尔数组，表示哪些锚点是正样本
        assigned_gt_inds[pos_inds] = argmax_overlaps[pos_inds] + 1 # argmax_overlaps[i] 是第 i 个锚点与真实目标框的最大 IoU 对应的目标框索引

        # 适当增加更多正样本
        # 对于每个 gt bbox 需要找出和其最大 iou 的 anchor 索引，如果其 iou 大于 min_pos_iou，
        # 则将该 anchor 对应索引的 assigned_gt_inds 设置为正样本，表示该 anchor 负责预测对应的 gt bbox。
        # 通过本步骤，可以最大程度保证每个 gt bbox 都有相应的 anchor 负责预测，但是如果其最大 iou 值还是小于 min_pos_iou，则依然不被认为是前景物体。
            ...
            for i in range(num_gts):
                if gt_max_overlaps[i] >= self.min_pos_iou:
                    if self.gt_max_assign_all:
                        max_iou_inds = overlaps[i, :] == gt_max_overlaps[i] # i为当前匹配的gt bbox编号
