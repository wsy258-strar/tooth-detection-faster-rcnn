...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def _predict_by_feat_single(self,
        ...
        """
        将单张图像经过模型头部网络提取的特征，通过特定计算（如回归、分类）转换为最终的边界框检测结果
        通常包括分类、回归和后处理
        ...
        Args:
            ...
            bbox_pred_list (list[Tensor]): Box energies / deltas from
                ...
                (num_priors * 4, H, W).
                bbox_pred：模型预测的偏移量（如 Δcx, Δcy, Δw, Δh）修正锚框，得到最终的检测框
            ...
            mlvl_priors (list[Tensor]): Each element in the list is
                ...
                (num_priors, 4).
                priors：[N, 4]，其中 N 是archor数量，每个锚框用 (cx, cy, w, h)（中心坐标、宽高）表示。
        ...
        for level_idx, (cls_score, bbox_pred, score_factor, priors) in \
            ...
            # in v3det.
            #对每层的分类 cls_score 进行 sigmoid 操作得到概率值
            ...
            # 对 5 个 head 输出特征图结果进行遍历，先按照预测分值排序，每个特征图都会保留前 nms_pre 个预测结果，
            # 最后所有特征图的预测结果会被合并，进行后续的处理。
            results = filter_scores_and_topk( # filter筛选
        ...
        # 对每张图片的五个输出层都进行sigmoid 操作得到概率值，按照分类预测分值排序，将预测结果全部收集，然后进行解码
    ...
    def _bbox_post_process(self,
        ...
        # 进行统一的 NMS 操作，每张图片最终保留 cfg.max_per_img 个预测框
